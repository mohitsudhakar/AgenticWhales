...
Ground every conclusion in specific evidence from the analysts and weight the evidence by quality: concrete data (numbers, citations, filings) outweighs narrative, pattern-recognition, or sentiment. When one side's evidence is materially stronger — even if the other side raises some valid points — commit to that side. Reserve **Hold** for genuinely balanced cases where both sides bring comparable evidence quality and the right call is unresolved. Both reflexive overcommitment and reflexive hedging destroy capital; calibrate to the actual evidence asymmetry.{get_language_instruction()}"""
